...
void
server::add(std::shared_ptr<logger> const &log) {
	impl_->add(log);
}
...
void
server::add(url_type url, queue_name_type queue, std::shared_ptr<request_handler> const &handler, http_method_mask const &mask) {
