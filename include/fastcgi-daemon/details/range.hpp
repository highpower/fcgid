...
#ifndef FASTCGI_DAEMON_DETAILS_RANGE_HPP_INCLUDED
#define FASTCGI_DAEMON_DETAILS_RANGE_HPP_INCLUDED
...
operator < (range<char const*> const &value, range<char const*> const &other) {
	return (!value.empty() && !other.empty()) ? (strncmp(value.begin(), other.begin(), std::max(value.size(), other.size())) < 0) : !other.empty();
}
...
#endif // FASTCGI_DAEMON_DETAILS_RANGE_HPP_INCLUDED
